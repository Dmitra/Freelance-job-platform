module AuthenticatedSystem
  ...
  def current_user
    @current_user ||= (login_from_session || login_from_basic_auth || login_from_cookie || VISITOR)
  end

  # Store the given user id in the session.
  def current_user=(new_user)
    session[:user_id] = (new_user.nil? || new_user.is_a?(Symbol)) ? nil : new_user.id 
    @current_user = new_user || VISITOR
  end
  ...
  def permission_denied      
    respond_to do |format|
      ...
      format.xml do
        ...
        render :text => "401 Unauthorized", :status => '401 Unauthorized'
      end
    end
  end
